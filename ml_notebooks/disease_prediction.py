...
np.ravel(y_test)
# TRAINING DATA
df=pd.read_csv("disease_Training.csv")
...
'(vertigo) Paroymsal  Positional Vertigo':36,'Acne':37,'Urinary tract infection':38,'Psoriasis':39,
'Impetigo':40}},inplace=True)
doc = pd.read_csv("doctor_list.csv")
prog = doc["prognosis"]
b = doc["Doctor"]
...
def NaiveBayes():
    ...
    if (h=='yes'):
        ...
        messagebox.showinfo("caution!!", "you might be suffering from " + c)
        messagebox.showinfo("caution!!", "please consult " + d)
    else:
        t3.delete("1.0", END)
        t3.insert(END, "No Disease")
        messagebox.showinfo("be happy", "you have no disease")
...
t3 = Text(root, height=2, width=35)
